...
export const crawlAndUpdateGlobal = async (outputPath, apiClient) => {
    let prevData = {};
    ...
    const res = await apiClient.findAllGlobalStat();
    console.log(res);
    const oldRows = res.result.filter(x => x.date === crawledDate);
